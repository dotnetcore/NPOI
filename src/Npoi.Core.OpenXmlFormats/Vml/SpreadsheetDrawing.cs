...
namespace Npoi.Core.OpenXmlFormats.Vml.Spreadsheet
{
    ...
    public class CT_ClientData
    {
        ...
        //    }
        //}
        public static CT_ClientData Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_ClientData ctObj = new CT_ClientData();
            if (node.Attribute("ObjectType") != null)
                ctObj.ObjectType = (ST_ObjectType)Enum.Parse(typeof(ST_ObjectType), node.Attribute("ObjectType").Value);
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "Anchor")
                    ...
                    ctObj.row.Add(Int32.Parse(childNode.Value));
            }
            return ctObj;
        }
