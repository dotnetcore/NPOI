...
namespace Npoi.Core.OpenXmlFormats.Spreadsheet
{
    ...
    public class CT_Col
    {
        ...
        public static CT_Col Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            ctObj.min = XmlHelper.ReadUInt(node.Attribute("min"));
            ctObj.max = XmlHelper.ReadUInt(node.Attribute("max"));
            ctObj.width = XmlHelper.ReadDouble(node.Attribute("width"));
            if (node.Attribute("style") != null)
                ctObj.style = XmlHelper.ReadUInt(node.Attribute("style"));
            else
                ctObj.style = null;
            ctObj.hidden = XmlHelper.ReadBool(node.Attribute("hidden"));
            ctObj.bestFit = XmlHelper.ReadBool(node.Attribute("bestFit"));
            ctObj.outlineLevel = XmlHelper.ReadByte(node.Attribute("outlineLevel"));
            ctObj.customWidth = XmlHelper.ReadBool(node.Attribute("customWidth"));
            ctObj.phonetic = XmlHelper.ReadBool(node.Attribute("phonetic"));
