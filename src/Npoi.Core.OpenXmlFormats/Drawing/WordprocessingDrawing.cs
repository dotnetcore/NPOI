...
namespace Npoi.Core.OpenXmlFormats.Dml.WordProcessing
{
    ...
    public class CT_Drawing
    {
        ...
        public static CT_Drawing Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                ...
                else if (childNode.Name.LocalName == "inline")
    ...
    public class CT_Inline
    {
        ...
        private Npoi.Core.OpenXmlFormats.Dml.WordProcessing.CT_NonVisualGraphicFrameProperties cNvGraphicFramePrField;
        ...
        public static CT_Inline Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_Inline ctObj = new CT_Inline();
            ctObj.distT = XmlHelper.ReadUInt(node.Attribute("wp:distT"));
            ctObj.distB = XmlHelper.ReadUInt(node.Attribute("wp:distB"));
            ctObj.distL = XmlHelper.ReadUInt(node.Attribute("wp:distL"));
            ctObj.distR = XmlHelper.ReadUInt(node.Attribute("wp:distR"));
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "extent")
                    ctObj.extent = CT_PositiveSize2D.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "effectExtent")
                    ctObj.effectExtent = CT_EffectExtent.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "docPr")
    ...
    public class CT_EffectExtent
    {
        ...
        public static CT_EffectExtent Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_EffectExtent ctObj = new CT_EffectExtent();
            ctObj.l = XmlHelper.ReadLong(node.Attribute("l"));
            ctObj.t = XmlHelper.ReadLong(node.Attribute("t"));
            ctObj.r = XmlHelper.ReadLong(node.Attribute("r"));
            ctObj.b = XmlHelper.ReadLong(node.Attribute("b"));
            return ctObj;
        }
    ...
    public class CT_Anchor
    {
        ...
        public static CT_Anchor Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            ...
            ctObj.distL = XmlHelper.ReadUInt(node.Attribute("wp:distL"));
            ctObj.distR = XmlHelper.ReadUInt(node.Attribute("wp:distR"));
            ctObj.simplePos1 = XmlHelper.ReadBool(node.Attribute("wp:simplePos1"));
            ctObj.relativeHeight = XmlHelper.ReadUInt(node.Attribute("wp:relativeHeight"));
            ctObj.behindDoc = XmlHelper.ReadBool(node.Attribute("wp:behindDoc"));
            ctObj.locked = XmlHelper.ReadBool(node.Attribute("wp:locked"));
            ctObj.layoutInCell = XmlHelper.ReadBool(node.Attribute("wp:layoutInCell"));
            ctObj.hidden = XmlHelper.ReadBool(node.Attribute("wp:hidden"));
            ctObj.allowOverlap = XmlHelper.ReadBool(node.Attribute("wp:allowOverlap"));
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "simplePos")
                    ctObj.simplePos = CT_Point2D.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "positionH")
                    ctObj.positionH = CT_PosH.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "positionV")
                    ctObj.positionV = CT_PosV.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "extent")
                    ctObj.extent = CT_PositiveSize2D.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "effectExtent")
                    ctObj.effectExtent = CT_EffectExtent.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "docPr")
    ...
    {
        public static CT_PosH Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_PosH ctObj = new CT_PosH();
            ctObj.posOffset = XmlHelper.ReadInt(node.Attribute("wp:posOffset"));
            if (node.Attribute("wp:align") != null)
                ctObj.align = (ST_AlignH)Enum.Parse(typeof(ST_AlignH), node.Attribute("wp:align").Value);
            if (node.Attribute("wp:relativeFrom") != null)
                ctObj.relativeFrom = (ST_RelFromH)Enum.Parse(typeof(ST_RelFromH), node.Attribute("wp:relativeFrom").Value);
            return ctObj;
        }
    ...
    {
        public static CT_PosV Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            ...
            ctObj.posOffset = XmlHelper.ReadInt(node.Attribute("wp:posOffset"));
            if (node.Attribute("wp:align") != null)
                ctObj.align = (ST_AlignV)Enum.Parse(typeof(ST_AlignV), node.Attribute("wp:align").Value);
            if (node.Attribute("wp:relativeFrom") != null)
                ctObj.relativeFrom = (ST_RelFromV)Enum.Parse(typeof(ST_RelFromV), node.Attribute("wp:relativeFrom").Value);
            return ctObj;
        }
