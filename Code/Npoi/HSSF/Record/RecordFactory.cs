    ...
    public class RecordFactory
    {
        ...
        private static Dictionary<short, I_RecordCreator> RecordsToMap(Type[] records)
        {
            ...
            for (int i = 0; i < records.Length; i++)
            {
                ...
                if (recClass.GetTypeInfo().IsAbstract)
                {
                ...
                try
                {
                    sid = (short)recClass.GetTypeInfo().GetField("sid").GetValue(null);
                }
        ...
        private static I_RecordCreator GetRecordCreator(Type recClass)
        {
            try
            {
                ...
                constructor = recClass.GetTypeInfo().GetConstructor(CONSTRUCTOR_ARGS);
                if (constructor != null)
            ...
            try
            {
                MethodInfo m = recClass.GetTypeInfo().GetMethod("Create", CONSTRUCTOR_ARGS);
