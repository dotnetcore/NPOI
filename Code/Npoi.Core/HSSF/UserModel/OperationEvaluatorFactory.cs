...
namespace Npoi.Core.HSSF.UserModel
{
    ...
    class OperationEvaluatorFactory
    {
        ...
        private static void Add(Hashtable m, Type ptgClass, Type evalClass)
        {
            ...
            if (!typeof(OperationEval).GetTypeInfo().IsAssignableFrom(evalClass))
            {
                throw new ArgumentException("Expected OperationEval subclass");
            }
            if (!evalClass.GetTypeInfo().IsPublic)
            {
                throw new Exception("Eval class must be public");
            }
            if (evalClass.GetTypeInfo().IsAbstract)
            {
            ...
            try
            {
                constructor = evalClass.GetTypeInfo().GetConstructor(OPERATION_CONSTRUCTOR_CLASS_ARRAY);
            }
            catch (Exception)
            {
                throw;
