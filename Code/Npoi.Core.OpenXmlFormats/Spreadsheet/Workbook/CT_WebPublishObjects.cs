...
namespace NPOI.OpenXmlFormats.Spreadsheet
{
    ...
    public class CT_WebPublishing
    {
        ...
        public static CT_WebPublishing Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            ctObj.css = XmlHelper.ReadBool(node.Attribute("css"));
            ctObj.thicket = XmlHelper.ReadBool(node.Attribute("thicket"));
            ctObj.longFileNames = XmlHelper.ReadBool(node.Attribute("longFileNames"));
            ctObj.vml = XmlHelper.ReadBool(node.Attribute("vml"));
            ctObj.allowPng = XmlHelper.ReadBool(node.Attribute("allowPng"));
            if (node.Attribute("targetScreenSize") != null)
                ctObj.targetScreenSize = (ST_TargetScreenSize)Enum.Parse(typeof(ST_TargetScreenSize), node.Attribute("targetScreenSize").Value);
