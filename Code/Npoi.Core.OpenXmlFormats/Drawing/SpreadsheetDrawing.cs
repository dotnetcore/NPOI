...
namespace Npoi.Core.OpenXmlFormats.Dml.Spreadsheet
{
    ...
    {
        public static CT_NonVisualDrawingProps Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            ...
            ctObj.descr = XmlHelper.ReadString(node.Attribute("descr"));
            ctObj.hidden = XmlHelper.ReadBool(node.Attribute("hidden"));
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "hlinkClick")
                    ctObj.hlinkClick = CT_Hyperlink.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "hlinkHover")
    ...
    public class CT_NonVisualGraphicFrameProperties
    {
        ...
        public static CT_NonVisualGraphicFrameProperties Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "graphicFrameLocks")
    ...
    public class CT_GraphicalObjectFrameNonVisual
    {
        ...
        public static CT_GraphicalObjectFrameNonVisual Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                ...
                else if (childNode.Name.LocalName == "cNvGraphicFramePr")
    ...
    {
        public static CT_NonVisualPictureProperties Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_NonVisualPictureProperties ctObj = new CT_NonVisualPictureProperties();
            ctObj.preferRelativeResize = XmlHelper.ReadBool(node.Attribute("preferRelativeResize"));
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "picLocks")
    ...
    public class CT_ShapeProperties
    {
        ...
        public static CT_ShapeProperties Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_ShapeProperties ctObj = new CT_ShapeProperties();
            if (node.Attribute("bwMode") != null)
                ctObj.bwMode = (ST_BlackWhiteMode)Enum.Parse(typeof(ST_BlackWhiteMode), node.Attribute("bwMode").Value);
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "xfrm")
                    ctObj.xfrm = CT_Transform2D.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "custGeom")
                    ctObj.custGeom = CT_CustomGeometry2D.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "prstGeom")
                ...
                else if (childNode.Name.LocalName == "ln")
    ...
    public class CT_GraphicalObjectFrame
    {
        ...
        public static CT_GraphicalObjectFrame Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "nvGraphicFramePr")
                    ctObj.nvGraphicFramePr = CT_GraphicalObjectFrameNonVisual.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "xfrm")
                    ctObj.xfrm = CT_Transform2D.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "graphic")
    ...
    public class CT_ConnectorNonVisual
    {
        ...
        public static CT_ConnectorNonVisual Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
    ...
    public class CT_Marker
    {
        ...
        public static CT_Marker Parse(XElement node, XmlNamespaceManager nameSpaceManager)
        {
            CT_Marker ctMarker = new CT_Marker();
            foreach (XElement subnode in node.ChildElements())
            {
                if (subnode.Name.LocalName == "col")
                {
                    ctMarker.col = Int32.Parse(subnode.Value);
                }
                else if (subnode.Name.LocalName == "colOff")
                {
                    ctMarker.colOff = Int64.Parse(subnode.Value);
                }
                else if (subnode.Name.LocalName == "row")
                {
                    ctMarker.row = Int32.Parse(subnode.Value);
                }
                else if (subnode.Name.LocalName == "rowOff")
                {
                    ctMarker.rowOff = Int64.Parse(subnode.Value);
                }
            }
            return ctMarker;
        }
    ...
    {
        public static CT_AnchorClientData Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_AnchorClientData ctObj = new CT_AnchorClientData();
            ctObj.fLocksWithSheet = XmlHelper.ReadBool(node.Attribute("fLocksWithSheet"));
            ctObj.fPrintsWithSheet = XmlHelper.ReadBool(node.Attribute("fPrintsWithSheet"));
            return ctObj;
        }
    ...
    public class CT_Drawing
    {
        ...
        public static CT_Drawing Parse(XDocument xmldoc, XmlNamespaceManager namespaceManager)
        {
            var cellanchorNodes = xmldoc.XPathSelectElements("/xdr:wsDr/*", namespaceManager);
            CT_Drawing ctDrawing = new CT_Drawing();
            foreach (XElement node in cellanchorNodes)
            {
                if (node.Name.LocalName == "twoCellAnchor")
                {
                    CT_TwoCellAnchor twoCellAnchor = CT_TwoCellAnchor.Parse(node, namespaceManager);
                    ctDrawing.cellAnchors.Add(twoCellAnchor);
                }
                else if (node.Name.LocalName == "oneCellAnchor")
                {
                    CT_OneCellAnchor oneCellAnchor = CT_OneCellAnchor.Parse(node, namespaceManager);
                    ctDrawing.cellAnchors.Add(oneCellAnchor);
                }
                else if (node.Name.LocalName == "absCellAnchor")
                {
    ...
    public class CT_AbsoluteCellAnchor : IEG_Anchor
    {
        ...
        public static CT_AbsoluteCellAnchor Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
    ...
    public class CT_TwoCellAnchor : IEG_Anchor //- was empty interface
    {
        ...
        private bool editAsSpecifiedField = false;
        ...
        internal static CT_TwoCellAnchor Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            CT_TwoCellAnchor twoCellAnchor = new CT_TwoCellAnchor();
            if (node.Attribute("editAs") != null)
                twoCellAnchor.editAs = (ST_EditAs)Enum.Parse(typeof(ST_EditAs), node.Attribute("editAs").Value);
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                ...
                else if (childNode.Name.LocalName == "to")
                {
    ...
    public class CT_Connector // empty interface: EG_ObjectChoices
    {
        ...
        public static CT_Connector Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "nvCxnSpPr")
