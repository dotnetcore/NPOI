...
namespace Npoi.Core.OpenXmlFormats.Wordprocessing
{
    ...
    public class CT_FramePr
    {
        ...
        public static CT_FramePr Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            if (node.Attribute("w:dropCap") != null)
                ctObj.dropCap = (ST_DropCap)Enum.Parse(typeof(ST_DropCap), node.Attribute("w:dropCap").Value);
            ctObj.lines = XmlHelper.ReadString(node.Attribute("w:lines"));
            ctObj.w = XmlHelper.ReadULong(node.Attribute("w:w"));
            ctObj.h = XmlHelper.ReadULong(node.Attribute("w:h"));
            ctObj.vSpace = XmlHelper.ReadULong(node.Attribute("w:vSpace"));
            ctObj.hSpace = XmlHelper.ReadULong(node.Attribute("w:hSpace"));
            if (node.Attribute("w:wrap") != null)
                ctObj.wrap = (ST_Wrap)Enum.Parse(typeof(ST_Wrap), node.Attribute("w:wrap").Value);
            if (node.Attribute("w:hAnchor") != null)
                ctObj.hAnchor = (ST_HAnchor)Enum.Parse(typeof(ST_HAnchor), node.Attribute("w:hAnchor").Value);
            if (node.Attribute("w:vAnchor") != null)
                ctObj.vAnchor = (ST_VAnchor)Enum.Parse(typeof(ST_VAnchor), node.Attribute("w:vAnchor").Value);
            ctObj.x = XmlHelper.ReadString(node.Attribute("w:x"));
            if (node.Attribute("w:xAlign") != null)
                ctObj.xAlign = (ST_XAlign)Enum.Parse(typeof(ST_XAlign), node.Attribute("w:xAlign").Value);
            ctObj.y = XmlHelper.ReadString(node.Attribute("w:y"));
            if (node.Attribute("w:yAlign") != null)
                ctObj.yAlign = (ST_YAlign)Enum.Parse(typeof(ST_YAlign), node.Attribute("w:yAlign").Value);
            if (node.Attribute("w:hRule") != null)
                ctObj.hRule = (ST_HeightRule)Enum.Parse(typeof(ST_HeightRule), node.Attribute("w:hRule").Value);
            if (node.Attribute("w:anchorLock") != null)
                ctObj.anchorLock = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attribute("w:anchorLock").Value);
            return ctObj;
        }
