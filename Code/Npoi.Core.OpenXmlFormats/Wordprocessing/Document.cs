...
namespace NPOI.OpenXmlFormats.Wordprocessing
{
    ...
    {
        public static CT_Document Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            ...
            {
                if (childNode.Name.LocalName == "body")
                    ctObj.body = CT_Body.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "background")
    ...
    public class CT_DocGrid
    {
        ...
        public static CT_DocGrid Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_DocGrid ctObj = new CT_DocGrid();
            if (node.Attribute("w:type") != null)
                ctObj.type = (ST_DocGrid)Enum.Parse(typeof(ST_DocGrid), node.Attribute("w:type").Value);
            ctObj.linePitch = XmlHelper.ReadString(node.Attribute("w:linePitch"));
            ctObj.charSpace = XmlHelper.ReadString(node.Attribute("w:charSpace"));
            return ctObj;
        }
