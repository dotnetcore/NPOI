...
namespace Npoi.Core.OpenXmlFormats.Wordprocessing
{
    ...
    public class CT_Fonts
    {
        ...
        public static CT_Fonts Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            ctObj.ascii = XmlHelper.ReadString(node.Attribute("w:ascii"));
            ctObj.hAnsi = XmlHelper.ReadString(node.Attribute("w:hAnsi"));
            ctObj.eastAsia = XmlHelper.ReadString(node.Attribute("w:eastAsia"));
            ctObj.cs = XmlHelper.ReadString(node.Attribute("w:cs"));
            if (node.Attribute("w:hint") != null && node.Attribute("w:hint").Value != "default")
                ctObj.hint = (ST_Hint)Enum.Parse(typeof(ST_Hint), node.Attribute("w:hint").Value);
            if (node.Attribute("w:asciiTheme") != null)
                ctObj.asciiTheme = (ST_Theme)Enum.Parse(typeof(ST_Theme), node.Attribute("w:asciiTheme").Value);
            if (node.Attribute("w:hAnsiTheme") != null)
                ctObj.hAnsiTheme = (ST_Theme)Enum.Parse(typeof(ST_Theme), node.Attribute("w:hAnsiTheme").Value);
            if (node.Attribute("w:eastAsiaTheme") != null)
                ctObj.eastAsiaTheme = (ST_Theme)Enum.Parse(typeof(ST_Theme), node.Attribute("w:eastAsiaTheme").Value);
            if (node.Attribute("w:cstheme") != null)
                ctObj.cstheme = (ST_Theme)Enum.Parse(typeof(ST_Theme), node.Attribute("w:cstheme").Value);
            return ctObj;
        }
