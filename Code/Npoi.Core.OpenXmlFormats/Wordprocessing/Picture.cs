...
namespace Npoi.Core.OpenXmlFormats.Wordprocessing
{
	...
	public class CT_PictureBase
	{
		...
		private List<XElement> itemsField;
		...
		public CT_PictureBase()
		{
			...
			this.itemsField = new List<XElement>();
		}
		...
		public void Set(object obj)
		{
			...
			var xmlDoc = XDocument.Load(output.ToString());
			lock (this)
			{
				this.itemsField.Add(xmlDoc.Document.Root.CloneNode(true));
	...
	public class CT_Picture : CT_PictureBase
	{
		...
		internal void Write(StreamWriter sw, string nodeName)
		{
			...
			foreach (XElement childnode in Items)
			{
	...
	public class CT_Background : CT_PictureBase
	{
		...
		public static CT_Background Parse(XElement node, XmlNamespaceManager namespaceManager)
		{
			if (node == null)
				return null;
	...
	public class CT_Object : CT_PictureBase
	{
		...
		public static CT_Object Parse(XElement node, XmlNamespaceManager namespaceManager)
		{
			if (node == null)
				return null;
			CT_Object ctObj = new CT_Object();
			ctObj.dxaOrig = XmlHelper.ReadULong(node.Attribute("w:dxaOrig"));
			ctObj.dyaOrig = XmlHelper.ReadULong(node.Attribute("w:dyaOrig"));
			foreach (XElement childNode in node.ChildElements())
			{
				if (childNode.Name.LocalName == "control")
					ctObj.control = CT_Control.Parse(childNode, namespaceManager);
			}
			return ctObj;
		}
	...
	{
		public static CT_Control Parse(XElement node, XmlNamespaceManager namespaceManager)
		{
			if (node == null)
				return null;
			CT_Control ctObj = new CT_Control();
			ctObj.name = XmlHelper.ReadString(node.Attribute("w:name"));
			ctObj.shapeid = XmlHelper.ReadString(node.Attribute("w:shapeid"));
