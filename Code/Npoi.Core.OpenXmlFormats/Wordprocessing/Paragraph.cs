...
namespace Npoi.Core.OpenXmlFormats.Wordprocessing
{
    ...
    public class CT_P
    {
        ...
        public static CT_P Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_P ctObj = new CT_P();
            ctObj.rsidRPr = XmlHelper.ReadBytes(node.Attribute("w:rsidRPr"));
            ctObj.rsidR = XmlHelper.ReadBytes(node.Attribute("w:rsidR"));
            ctObj.rsidDel = XmlHelper.ReadBytes(node.Attribute("w:rsidDel"));
            ctObj.rsidP = XmlHelper.ReadBytes(node.Attribute("w:rsidP"));
            ctObj.rsidRDefault = XmlHelper.ReadBytes(node.Attribute("w:rsidRDefault"));
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                ...
                else if (childNode.Name.LocalName == "fldSimple")
                {
                ...
                else if (childNode.Name.LocalName == "hyperlink")
                {
                ...
                else if (childNode.Name.LocalName == "smartTag")
                {
                ...
                else if (childNode.Name.LocalName == "subDoc")
                {
    ...
    public class CT_ParaRPr
    {
        ...
        public static CT_ParaRPr Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "ins")
                ...
                else if (childNode.Name.LocalName == "rStyle")
                    ctObj.rStyle = CT_String.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "rFonts")
                ...
                else if (childNode.Name.LocalName == "bCs")
                ...
                else if (childNode.Name.LocalName == "iCs")
                    ctObj.iCs = CT_OnOff.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "caps")
                    ctObj.caps = CT_OnOff.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "smallCaps")
                ...
                else if (childNode.Name.LocalName == "dstrike")
                ...
                else if (childNode.Name.LocalName == "w")
                    ctObj.w = CT_TextScale.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "kern")
                    ctObj.kern = CT_HpsMeasure.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "position")
                ...
                else if (childNode.Name.LocalName == "szCs")
                ...
                else if (childNode.Name.LocalName == "effect")
                    ctObj.effect = CT_TextEffect.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "bdr")
                    ctObj.bdr = CT_Border.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "shd")
                    ctObj.shd = CT_Shd.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "fitText")
                ...
                else if (childNode.Name.LocalName == "rtl")
    ...
    public class CT_SectPrChange : CT_TrackChange
    {
        ...
        public static new CT_SectPrChange Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "sectPr")
    ...
    public class CT_SectPrBase
    {
        ...
        public static CT_SectPrBase Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            ctObj.rsidDel = XmlHelper.ReadBytes(node.Attribute("w:rsidDel"));
            ctObj.rsidR = XmlHelper.ReadBytes(node.Attribute("w:rsidR"));
            ctObj.rsidSect = XmlHelper.ReadBytes(node.Attribute("w:rsidSect"));
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "footnotePr")
    ...
    public class CT_SectPr
    {
        ...
        public static CT_SectPr Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "footnotePr")
                    ...
                    ctObj.printerSettings = CT_Rel.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "sectPrChange")
                    ctObj.sectPrChange = CT_SectPrChange.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "footerReference")
                    ctObj.footerReference.Add(CT_HdrFtrRef.Parse(childNode, namespaceManager));
                else if (childNode.Name.LocalName == "headerReference")
    ...
    public class CT_PBdr
    {
        ...
        public static CT_PBdr Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "top")
                    ctObj.top = CT_Border.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "left")
                ...
                else if (childNode.Name.LocalName == "between")
                    ctObj.between = CT_Border.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "bar")
    ...
    public class CT_Spacing
    {
        ...
        public static CT_Spacing Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            ctObj.before = XmlHelper.ReadULong(node.Attribute("w:before"));
            ctObj.beforeLines = XmlHelper.ReadString(node.Attribute("w:beforeLines"));
            if (node.Attribute("w:beforeAutospacing") != null)
                ctObj.beforeAutospacing = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attribute("w:beforeAutospacing").Value);
            ctObj.after = XmlHelper.ReadULong(node.Attribute("w:after"));
            ctObj.afterLines = XmlHelper.ReadString(node.Attribute("w:afterLines"));
            if (node.Attribute("w:afterAutospacing") != null)
                ctObj.afterAutospacing = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attribute("w:afterAutospacing").Value);
            ctObj.line = XmlHelper.ReadString(node.Attribute("w:line"));
            if (node.Attribute("w:lineRule") != null)
                ctObj.lineRule = (ST_LineSpacingRule)Enum.Parse(typeof(ST_LineSpacingRule), node.Attribute("w:lineRule").Value);
            return ctObj;
        }
    ...
    public class CT_Ind
    {
        ...
        public static CT_Ind Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_Ind ctObj = new CT_Ind();
            ctObj.left = XmlHelper.ReadString(node.Attribute("w:left"));
            ctObj.leftChars = XmlHelper.ReadString(node.Attribute("w:leftChars"));
            ctObj.right = XmlHelper.ReadString(node.Attribute("w:right"));
    ...
    public class CT_RPr
    {
        ...
        public static CT_RPr Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "rStyle")
                    ctObj.rStyle = CT_String.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "rFonts")
                    ctObj.rFonts = CT_Fonts.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "b")
    ...
    {
        public static new CT_ParaRPrChange Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            if (node == null)
                return null;
            CT_ParaRPrChange ctObj = new CT_ParaRPrChange();
            ctObj.author = XmlHelper.ReadString(node.Attribute("author"));
            ctObj.date = XmlHelper.ReadString(node.Attribute("date"));
...
cnfStyleField == null;
        ...
        public static CT_PPrBase Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "pStyle")
                    ctObj.pStyle = CT_String.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "keepNext")
                    ctObj.keepNext = CT_OnOff.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "keepLines")
    ...
    public class CT_RPrOriginal
    {
        ...
        public static CT_RPrOriginal Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "spacing")
                    ctObj.spacing.Add(CT_SignedTwipsMeasure.Parse(childNode, namespaceManager));
                else if (childNode.Name.LocalName == "vertAlign")
                    ctObj.vertAlign.Add(CT_VerticalAlignRun.Parse(childNode, namespaceManager));
                else if (childNode.Name.LocalName == "w")
    ...
    public class CT_Br
    {
        ...
        public static CT_Br Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            if (node.Attribute("w:type") != null)
                ctObj.type = (ST_BrType)Enum.Parse(typeof(ST_BrType), node.Attribute("w:type").Value);
            if (node.Attribute("w:clear") != null)
    ...
    public class CT_TblStylePr
    {
        ...
        public static CT_TblStylePr Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            if (node.Attribute("w:type") != null)
                ctObj.type = (ST_TblStyleOverrideType)Enum.Parse(typeof(ST_TblStyleOverrideType), node.Attribute("w:type").Value);
            foreach (XElement childNode in node.ChildElements())
            {
                if (childNode.Name.LocalName == "pPr")
                    ctObj.pPr = CT_PPr.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "rPr")
                    ctObj.rPr = CT_RPr.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "tblPr")
                    ctObj.tblPr = CT_TblPrBase.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "trPr")
                    ctObj.trPr = CT_TrPr.Parse(childNode, namespaceManager);
                else if (childNode.Name.LocalName == "tcPr")
