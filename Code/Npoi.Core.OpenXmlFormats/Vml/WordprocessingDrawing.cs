...
namespace NPOI.OpenXmlFormats.Vml.Wordprocessing
{
    ...
    public class CT_Wrap {
        ...
        public static CT_Wrap Parse(XElement node, XmlNamespaceManager namespaceManager)
        {
            ...
            if (node.Attribute("type") != null)
                ctObj.type = (ST_WrapType)Enum.Parse(typeof(ST_WrapType), node.Attribute("type").Value);
            if (node.Attribute("side") != null)
                ctObj.side = (ST_WrapSide)Enum.Parse(typeof(ST_WrapSide), node.Attribute("side").Value);
            if (node.Attribute("anchorx") != null)
                ctObj.anchorx = (ST_HorizontalAnchor)Enum.Parse(typeof(ST_HorizontalAnchor), node.Attribute("anchorx").Value);
            if (node.Attribute("anchory") != null)
                ctObj.anchory = (ST_VerticalAnchor)Enum.Parse(typeof(ST_VerticalAnchor), node.Attribute("anchory").Value);
            return ctObj;
        }
